#Harshvardhan Vala
#Roll No: 20110075
#BTECH First Year, CSE
#ES102 Project
#Musical Hangman Game code:
...
#Here I'm calling the Intro theme. I need it to play only for 5 seconds
...
positions_of_alphabets=[]   #contains x,y coordinates of the alphabet buttons
leftmost_position=60
rightmost_poisiton=720
gap_between_postions=((rightmost_poisiton-leftmost_position)//12)
print(leftmost_position,rightmost_poisiton,gap_between_postions)
for i in range(13):
    x=leftmost_position+i*gap_between_postions
    y=400
...
for i in range(26):
    main_button_controller.append([positions_of_alphabets[i][0],positions_of_alphabets[i][1],Alphabets[i]])
...
def buttons():
    for elements in main_button_controller:
        x,y,alpha=elements                                          
        pygame.draw.rect(screen,black,(x-17,y-17,40,40),2)
        FONT=pygame.font.SysFont('Bradley Hand ITC.tff',40)
        buttonletters=FONT.render(alpha,True,black)
...
for i in range(len(randomword)):                 # This also tracks the record of correctly guessed letters by far.
...
while run:
    ...
    for event in pygame.event.get():
        if event.type==pygame.QUIT:
            quit()
        #To get get the input from user using buttons displayed on the screen:
        ...
        #To get the input from user from keyboard buttons                
        if event.type==pygame.KEYDOWN:
            for elements in main_button_controller:
                x_coordinate,y_coordinate,character=elements
                if event.unicode==character or event.unicode==character.lower():
                    Guessed_letters.append(character.upper())
                    for guesses in Guessed_letters:
                        if guesses in sorted_letter_of_randomword:
                            correctly_guessed_letters.append(guesses)
